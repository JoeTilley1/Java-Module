...
class Newton {
    ...
    public void iterate(Complex z0) {
        // You need to fill in this method.
        int iter = 0;                                       //Number of iterations gone through
        Complex[] rootapprox = new Complex[MAXITER + 1];    //Stores array of root approximations
        rootapprox[0] = z0;
        while(true) {
            if (f.evaluate(z0) == new Complex()) {          //Break if root is achieved
                err = 0;
                root = z0;
                numIterations = 0;
                break;
            }
            if (fp.evaluate(z0) == new Complex()) {         //Break if error is achieved
                err = -1;
                break;
            }
            if (iter >= MAXITER) {                          //Break if max iterations is achieved
                err = -2;
                break;
            }
            //Do Newton Method
            rootapprox[iter + 1] = rootapprox[iter].add((f.evaluate(rootapprox[iter]).multiply(-1)).divide(fp.evaluate(rootapprox[iter])));
            iter = iter + 1;    //Add 1 to number of iterations
            if ((rootapprox[iter].add(rootapprox[iter - 1].multiply(-1))).abs() < TOL) {    //If defining tolerance method is achieved, break
                err = 0;
                root = rootapprox[iter];
                numIterations = iter;
                break;
            }
        }
